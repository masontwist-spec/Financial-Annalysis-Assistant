  ...
  return (
    <div className="space-y-8">
      <div className="pixel-panel p-6 sm:p-8">
        <p className="text-sm font-black uppercase tracking-[0.25em] text-neutral-700">Dashboard</p>
        <h1 className="pixel-title mt-3 text-4xl font-black uppercase">Portfolio research overview</h1>
        <p className="mt-4 max-w-2xl font-bold text-neutral-800">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {metrics.map((metric) => (
          <article key={metric.label} className="pixel-card p-5">
            <p className="text-sm font-black uppercase text-neutral-700">{metric.label}</p>
      ...
      <section className="grid gap-4 xl:grid-cols-[1.4fr_0.6fr]">
        <article className="pixel-panel p-6 sm:p-8">
          ...
          <p className="mt-3 font-bold text-neutral-800">Your latest analysis items are listed below for quick follow-up.</p>
          <ul className="mt-6 space-y-4">
            {['Alpha Dynamics', 'GreenEdge Holdings', 'BluePeak Tech'].map((company) => (
              <li key={company} className="border-2 border-black bg-white p-4 shadow-pixel">
        ...
        <article className="pixel-panel p-6 sm:p-8">
          ...
          <p className="mt-3 font-bold text-neutral-800">Move through gate checkpoints for each company to build a stronger investment thesis.</p>
